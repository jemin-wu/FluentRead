...
describe('translator placeholder system', () => {
  ...
  describe('extractPlaceholders', () => {
    ...
    it('extracts code tags as placeholders with outerHTML', () => {
      document.body.innerHTML = '<p>Use <code>git pull</code> to update</p>';
      const el = document.querySelector('p')!;
      const result = extractPlaceholders(el);

      expect(result.placeholders).toHaveLength(1);
      expect(result.placeholders[0].tag).toBe('code');
      expect(result.placeholders[0].content).toBe('<code>git pull</code>');
      expect(result.text).toContain('__TAG_0__');
    });

    it('extracts code tags preserving original class attribute', () => {
      document.body.innerHTML = '<p>Use <code class="language-js">foo</code> here</p>';
      const el = document.querySelector('p')!;
      const result = extractPlaceholders(el);

      expect(result.placeholders).toHaveLength(1);
      expect(result.placeholders[0].content).toBe('<code class="language-js">foo</code>');
    });

    it('extracts sup and sub tags with outerHTML', () => {
      document.body.innerHTML = '<p>H<sub>2</sub>O is water<sup>1</sup></p>';
      const el = document.querySelector('p')!;
      const result = extractPlaceholders(el);

      expect(result.placeholders).toHaveLength(2);
      expect(result.placeholders[0]).toEqual({ tag: 'sub', content: '<sub>2</sub>' });
      expect(result.placeholders[1]).toEqual({ tag: 'sup', content: '<sup>1</sup>' });
    });
    ...
    it('preserves code tags nested inside links (code processed before links)', () => {
      ...
      // code should be extracted as placeholder with outerHTML
      expect(result.placeholders).toHaveLength(1);
      expect(result.placeholders[0].tag).toBe('code');
      expect(result.placeholders[0].content).toBe('<code>npm install</code>');
  ...
  describe('restorePlaceholders', () => {
    ...
    it('restores code tags in translation using original outerHTML', () => {
      const result = restorePlaceholders(
        '使用 __TAG_0__ 来更新',
        [{ tag: 'code', content: '<code>git pull</code>' }],
        [],
      );

      expect(result).toContain('<code>git pull</code>');
      expect(result).toContain('使用');
    });

    it('restores code tags preserving original class', () => {
      const result = restorePlaceholders(
        '使用 __TAG_0__ 来更新',
        [{ tag: 'code', content: '<code class="language-js">foo</code>' }],
        [],
      );

      expect(result).toContain('<code class="language-js">foo</code>');
    });

    it('restores sup/sub with original outerHTML', () => {
      const result = restorePlaceholders(
        'H__TAG_0__O 是水__TAG_1__',
        [
          { tag: 'sub', content: '<sub>2</sub>' },
          { tag: 'sup', content: '<sup>1</sup>' },
        ],
        [],
      );
    ...
    it('restores code inside links when both placeholders and links exist', () => {
      const result = restorePlaceholders(
        ...
        [{ tag: 'code', content: '<code>npm install</code>' }],
        [{ attrs: 'href="https://x.com"' }],
      );

      // TAG_0 restored to original outerHTML FIRST, then link wraps it
